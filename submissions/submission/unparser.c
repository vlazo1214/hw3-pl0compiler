/* $Id: unparser.c,v 1.6 2023/02/20 03:55:32 leavens Exp $ */
#include <stdio.h>
#include "ast.h"
#include "utilities.h"
...
{
    indent(out, level);
    fprintf(out, "%s := ", stmt->data.assign_stmt.name);
...
{
    indent(out, level);
    fprintf(out, "read %s", stmt->data.read_stmt.name);
    newlineAndOptionalSemi(out, addSemiToEnd);
}
